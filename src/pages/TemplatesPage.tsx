...
export function TemplatesPage() {
  ...
  return <div className="grid gap-7">
    ...
    {query.isLoading ? <div className="grid gap-4 md:grid-cols-3">{[1,2,3].map((i) => <div key={i} className="h-56 animate-pulse rounded-2xl bg-white" />)}</div> : query.data?.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{query.data.map((template) => <Card key={template.id} className="group p-6">
